...
const PlayerContent = ({ song, songUrl }) => {
  ...
  }, [playedSongs, player, setPlayedDuration, shuffle, updatePlayCount, user])
  ...
  const onPlayPrevious = () => {
    ...
    //   }
    // }
    if (shuffle && playedSongs.length > 0) {
      const previousSong = playedSongs[playedSongs.length - 1]; // Last played song
      setPlayedSongs(playedSongs.slice(0, -1)); // Remove it from the played songs list
      player.setId(previousSong);
    } else {
      const currentIndex = player.ids.findIndex((id) => id === player.activeId);
      const previousSong = player.ids[currentIndex - 1];
